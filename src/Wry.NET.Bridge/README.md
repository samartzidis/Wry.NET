...
| Built-in Tray Service | `bridge.RegisterTrayService(app)` registers the Tray service. JS can get tray IDs; set icon (base64), tooltip, title, visibility, menu-on-left-click, icon-as-template; get/set menu item text/enabled/checked; append/remove menu items. Subscribe to `trayMenuItemClicked` and `trayIconEvent` (Click, DoubleClick, Enter, Move, Leave). Trays are created from C# before Run (creating new trays from JS is not supported); you can change the icon and all other properties from JS. |
| Built-in Window Service | `bridge.RegisterWindowService(app)` registers the Window service. Pass **0** for windowId to target the calling window. Subscribe to `windowFocusChanged`, `windowResized`, `windowMoved`, `windowCloseRequested` (e.g. `onWindowFocusChanged(cb)`) to receive window events in JS; payloads include `windowId`. |
